...
// The TargetReticle class is based on the TargetLockReticle implementation from 'True Directional Movement':
// https://github.com/ersh1/TrueDirectionalMovement
// All credits go to the original author Ersh!

namespace DTR
{
	void TargetReticle::Update(float a_deltaTime)
	{
		if (_interpTimer > 0) {
	...
	void TargetReticle::WidgetReadyToRemove()
	{
		AddWidgetTask([=]() {
			RE::GFxValue arg;
			arg.SetBoolean(true);
	...
	void TargetReticle::Dispose()
	{
		_object.Invoke("cleanUp", nullptr, nullptr, 0);
	}

	void TargetReticle::SetWidgetState(WidgetState a_widgetState)
	{
	...
	void TargetReticle::ChangeTarget(RE::ObjectRefHandle a_refHandle, RE::NiPointer<RE::NiAVObject> a_targetPoint)
	{
	...
	void TargetReticle::UpdatePosition()
	{
	...
	void TargetReticle::UpdateWidgetState()
	{
	...
	void TargetReticle::StartInterpolation(InterpMode a_interpMode)
	{
		RE::GFxValue::DisplayInfo displayInfo;
	...
	void TargetReticle::UpdateState(bool a_isReticleLocked, bool a_isTDMLocked, int a_combatState)
	{
	...
	void TargetReticle::SetReticleLockAnimationStyle(int a_style)
	{
